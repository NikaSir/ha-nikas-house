...
Панель «Учёт воды» остаётся автономной и принадлежит репозиторию
`ha-water-accounting`; House содержит только явную внешнюю ссылку на
`/dashboard-water`.
...
Все проверки объединены в `repository-checks.yml` и запускаются при push в любую
ветку, pull request и вручную. Итоговая проверка `validate` успешна только после
успешного завершения всех трёх задач: `repository-checks`, `hacs` и `hassfest`.
Ошибка, пропуск или отмена любой из них не засчитываются как успех. Это проверка
кода и упаковки; приёмка панели в Home Assistant и на телефоне выполняется отдельно.

Ветка `main` защищена: слияние разрешено только после успешного агрегирующего
статуса `validate`.
